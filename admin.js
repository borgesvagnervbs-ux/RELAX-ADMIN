...
async function getDayAvailability(dateStr) {
  try {
    const doc = await firebase.firestore().collection('availability').doc(dateStr).get();
...
async function saveDayAvailability(dateStr, availability) {
  try {
    await firebase.firestore().collection('availability').doc(dateStr).set(availability);
  } catch (error) {
  ...
  if (slots.length === 0) {
    hourlyList.innerHTML = `
      <div class="no-slots-alert">
        ...
        <div class="no-slots-content">
          <h4>Dia não disponível</h4>
          <p>Este dia não está configurado para atendimento. Configure os horários na aba "Horários de Atendimento".</p>
        </div>
      </div>
    `;
    return;
...
function renderBackupScheduleForm() {
  ...
  schedule.forEach((item, idx) => {
    ...
    timeInput.disabled = !!schedule[0].interval && idx > 0;
    ...
    removeBtn.disabled = !!schedule[0].interval && idx > 0;
    removeBtn.onclick = () => {
...
setInterval(checkAndRunScheduledBackups,30000);
